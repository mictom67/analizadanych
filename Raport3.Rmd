...
Przydatne funkcje podczas wybierania/usuwania kolumn: 
 ...
 kolumny ktore zaczynaja sie od 
```{r piaty, eval=FALSE}
dane %>%
...
wybieramy i usuwamy kolumny ktorym odpowiada tekstowe, kolumny nazwy 5,6 liter  
```{r osmy, eval=FALSE}
dane %>%
...
dla mniejszych równych 9 i wiekszych od 1000 ! odwraca wtf 
```{r 18, eval=FALSE}
dane %>%
  filter(!(IMDb.Score >= 9) & !(IMDb.Votes < 1000))%>% 
  head(10)
...
jeseli wiekszy lub rowny to good, w przeciewnym poor mutate- kolumna istanieje tymczasowo w tej naszej funkcji 
```{r 19, eval=FALSE}
dane %>%
...
zmieniamy stary gowno zapis dat na nowy 
```{r 20, eval=FALSE}
dane %>%
  transmute(
...
dane %>%
  ...
  mutate(Netflix.Release = as.Date(Netflix.Release.Date, format = '%d/%m/%y')) %>%
  arrange(Netflix.Release) %>%
  select(Title, Netflix.Release) %>%
  head(1)
```{r 21, eval=FALSE}
# miejsce na kod
dane %>%
...
```{r 34, eval=FALSE}
dane %>%
  filter(str_detect(Genre, 'Comedy') & str_detect(Languages, 'Polish')) %>%
  arrange(-IMDb.Score) %>%
  select(Title, Genre, IMDb.Score) %>%
  head(3)
```
...
dane %>%
  mutate(
    Release = as.Date(Release.Date, format = '%m/%d/%y')
    ,Netflix.Release = as.Date(Netflix.Release.Date, format = '%m/%d/%y')
    ,Days.To.Netflix = as.numeric(Netflix.Release - Release)
  ) %>%
  filter(between(lubridate::year(Release), 2019, 2020)) %>%
  summarize(avg_days_to_netflix = mean(Days.To.Netflix, na.rm = TRUE)) %>%
  round(0)
